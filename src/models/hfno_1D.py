...
class HFNO_1D(nn.Module):
    def __init__(self, layer_modes, depth, width_MLP, n_layers_MLP, input_size, output_size, linear_W=True, device="cpu"):
        ...
        self.P = nn.Linear(self.input_size, self.depth, device=self.device)

        if self.linear_W:
            self.W = nn.Conv1d(self.depth, 1, 1, device=self.device)
        ...
        for i in range(len(self.layer_modes)):
            ...
            self.sublayers.append(MLP.FFNN(layers_width, device=self.device))
    ...
    def forward(self, x, indexes=None, sum_w=True):
        ...
        for i in range(len(self.layer_modes)):
            ...
            output_pad = torch.zeros(sh[0], m, dtype=torch.cfloat, device=self.device)
        ...
        if indexes.shape[0]==0:
            z = torch.zeros_like(torch.fft.irfft(output_pad, n=m), device=self.device)
        else:
