...
class IntegralKernel1D(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, modes1, device="cpu"):
        ...
        self.weights = nn.Parameter(self.scale * torch.rand(in_channels, out_channels, self.modes1, dtype=torch.cfloat, device=device))
...
class Fourier_layer1D(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, f_modes, l, device="cpu"):
        super(Fourier_layer1D, self).__init__()

        self.integral_kernel = IntegralKernel1D(in_channels, out_channels, f_modes, device=device)
        self.linear_transformation = nn.Conv1d(in_channels, out_channels, l, padding="same", device=device)
...
class FNO1D(nn.Module):
    def __init__(self, modes, width, l, n_layer=4, hidden_proj=None, device="cpu"):
        ...
        self.P = nn.Linear(1, self.width, device=device)  # input channel is 5: (u(x,y),v(x,y),w,(x,y) x, y)
        
        
        # n_l sequential layers
        self.layers = nn.ModuleList()
        for i in range(self.n_layer):
            self.layers.append(Fourier_layer1D(self.width, self.width, self.modes, l, device=device))


        # Projection layer
        self.Q = nn.Sequential(nn.Linear(self.width, self.hidden_proj, device=device), self.activation, nn.Linear(self.hidden_proj, 1, device=device))
